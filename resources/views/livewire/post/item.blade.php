<div class="max-w-lg mx-auto" x-data={}>
    ...
    <main>
        <div class="swiper h-[500px] border bg-white"
            x-init="
                new Swiper($el, {
            ...
            @if($post->media->count() > 1)
                {{-- prev nav --}}
                <div class="swiper-button-prev absolute top-1/2 left-1 z-10">
                    <div class="bg-white/95 border p-1 rounded-full text-gray-900">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.8" stroke="currentColor" class="size-6 w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
                        </svg>
                    </div>
                </div>
                {{-- next nav --}}
                <div class="swiper-button-next absolute top-1/2 right-1 z-10">
                    <div class="bg-white/95 border p-1 rounded-full text-gray-900">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.8" stroke="currentColor" class="size-6 w-4 h-4">
    ...
    <footer>
        <div class="flex gap-4 items-center my-2">
            ...
            @if($post->allow_commenting)
                <span onclick="Livewire.dispatch(
            ...
            {{-- forward --}}
            <span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" class="w-5 h-5" fill="currentColor" class="bi bi-send" viewBox="0 0 16 16">
        ...
        @if($post->totalLikers > 0 && !$post->hide_like_view)
            <p class="font-bold text-sm">{{ $post->totalLikers }} likes</p>
        @endif
        ...
        @if($post->allow_commenting)
            ...
            <button onclick="Livewire.dispatch(
                'openModal',{component: 'post.view.modal', arguments: {'post': {{ $post->id }}}}
                )" class="text-slate-500/90 text-sm font-medium">
                View all {{ $comment_count }} comments
            </button>

            @auth
                <ul class="my-2">
                    @foreach($post->comments()->where('user_id', auth()->user()->id)->get() as $comment)
                    <li class="grid grid-cols-12 text-sm items-center">
                        ...
                        <button wire:click="toggleCommentLike({{ $comment->id }})" class="col-span-1 mb-auto flex justify-end pr-px">
                            @if($comment->isLikedBy(auth()->user()))
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="red" class="w-3 h-3">
                                    <path d="m11.645 20.91-.007-.003-.022-.012a15.247 15.247 0 0 1-.383-.218 25.18 25.18 0 0 1-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0 1 12 5.052 5.5 5.5 0 0 1 16.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 0 1-4.244 3.17 15.247 15.247 0 0 1-.383.219l-.022.012-.007.004-.003.001a.752.752 0 0 1-.704 0l-.003-.001Z" />
                                </svg>
                            @else
            ...
            <form
                ...
                <div class="col-span-1 ml-auto flex justify-end text-right">
                    <button x-cloak x-show="body.length > 0" class="text-sm font-semibold flex justify-end text-blue-500">Post</button>
                </div>
