...
x-init="
 $wire.shrink={{request()->routeIs('chats')|| request()->routeIs('chats.main')}}
"
...
<ul class="space-y-4 mt-2">
    ...
    <li><a wire:navigate href="{{ route('chats') }}"
        class="flex items-center gap-5">

            <span>
                @if (request()->routeIs('chats') || request()->routeIs('chats.main'))
...
<div x-show="drawer" x-cloak x-transition.origin.left
    ...
    class="fixed inset-y-0 left-[70px] w-96 bg-white overflow-x-hidden overflow-y-scroll shadow px-4 border rounded-r-2xl z-100">
    <template x-if="showSearch">
        <div x-cloak class="h-full">
            <header class="sticky top-0 w-full bg-white py-2 z-20">
                <h5 class="text-4xl font-bold my-4">Search</h5>
                {{-- input --}}
                <input wire:model.live="query" type="search" class="border-0 outline-none w-full focus:outline-none bg-gray-100 rounded-lg hover:ring-0 focus:ring-0">
            </header>
            ...
            <main>
                ...
                <ul class="space-y-2 overflow-x-hidden">
                    ...
                    <li>
                        <a href="{{ route('profile.home', $user->user_name) }}" class="flex gap-2 truncate items-center">
                            <x-avatar wire:ignore class="w-9 h-9 mb-auto" src="https://placehold.co/600x600"/>
                            <div class="flex flex-col">
                                <span class="font-bold text-sm">{{ $user->user_name }}</span>
    ...
    <template x-cloak x-if="showNotifications">
